...
- **Devoluciones** parciales o totales con restauración automática de stock y reporte del día
...
- **Conexión WiFi** integrada: escaneo de redes y conexión por contraseña sin salir de la aplicación
- **Menú categorizado** (Ventas, Reportes, Sistema) con colores y estilo tipo tmux
- **Paginación** en todas las pantallas de listas para optimizar pantallas pequeñas (800x600)
...
│       ├── returns.go               # Pantalla de devoluciones
│       ├── returns_report.go        # Reporte de devoluciones del día
│       ├── shrinkage.go             # Registro de merma
│       ├── reports.go               # Reportes de cierre y reorden
│       ├── inventory.go             # Reporte de inventario
│       ├── monthly_finance.go       # Finanzas mensuales con gráfica
│       ├── wifi.go                  # Conexión a redes WiFi
...
| `sale_id` | INTEGER FK | Referencia a `sales` |
| `total` | REAL | Monto total devuelto |
...
|---|---|---|
| `id` | INTEGER PK | Identificador |
| `return_id` | INTEGER FK | Referencia a `returns` |
| `sale_item_id` | INTEGER FK | Referencia a `sale_items` |
| `product_id` | INTEGER FK | Referencia a `products` |
| `quantity` | REAL | Cantidad devuelta |
| `subtotal` | REAL | Subtotal reembolsado |

### Sistema de migraciones

La base de datos usa un sistema de migraciones versionadas con la tabla `schema_version`. Al iniciar, la aplicación verifica la versión actual y aplica las migraciones pendientes automáticamente. La versión actual del esquema es la 3 (incluye las tablas de devoluciones).
...
El menú principal está organizado en tres categorías con código de color tipo tmux:

- **Ventas** (verde): Nueva Venta, Devoluciones, Devoluciones del Día
- **Reportes** (azul): Cierre del Día, Reorden, Inventario, Finanzas Mensuales
- **Sistema** (naranja): Productos, Registrar Merma, Buscar Producto, Conectarse a WiFi
...
### Devoluciones
Selección de venta por ID, marcar líneas a devolver (parcial o total). Restaura stock automáticamente y registra el motivo.

### Devoluciones del día
Reporte de todas las devoluciones registradas en el día con total reembolsado.

### Productos
Lista paginada con precio, stock y tipo. Crear, editar y eliminar productos.
...
### Finanzas mensuales
Gráfica de barras con ingresos, costos y ganancia por mes. Vista comparativa anual.

### Conectarse a WiFi
Escaneo de redes disponibles, selección con cursor, ingreso de contraseña y conexión sin salir de la aplicación. Útil para configurar el Raspberry Pi en una red nueva sin teclado/monitor adicionales.
...
- **Resolución**: la UI está optimizada para pantallas de 800x600 (~100x37 caracteres) y se adapta a tamaños mayores

### Hardware recomendado

- Raspberry Pi 4 (4 GB RAM) o superior
- MicroSD de 16 GB o más
- Pantalla pequeña (800x600) o conexión SSH para acceso remoto
...
```bash
git clone https://github.com/oscarcn1/rpi-pos.git pos
cd pos
```
